...
void DetermineProperties(vector <Layer>& _Layers)
{// function to set the properties of each member of the vector of layers
	unsigned int _NoL = GetNoL();
	unsigned int _NoPL = GetNoPL();
	bool _IsOil = GetIsOil();
	...
	{
		cout<<"Please enter the properties for layer "<<_layerCounter+1<<"."<<endl;
		string _type = "N/A";
		while(_type != "substrate"||"protein"||"oil"||"aqueous")
		{
			cout<<"Please enter the type of layer number "<<_layerCounter<<": (substrate, protein, oil, or aqueous)"<<endl;
			cin>>_type;
		};
...
void MatixMethod(std::vector <double>& _Layers)
{
    ...
    {// calculate the layer matrix elements on a layer by layer basis
        ...
        {// for each Q value
            executeMatrixMethod(std::vector <double>& _Layers, unsigned int layerCounter)
            calculateCumulativeReflectance(unsigned int layerCounter, unsigned int dataPoint);
...
void executeMatrixMethod(vector < double >& _Layers, unsigned int _layerCounter)
...
void calculateCumulativeReflectance(vector < double >& _Layers, unsigned int _layerCounter)
...
void graphDataErr(vector< vector< double > >& _data, string _InFileName, unsigned int _NumberOfPoints)
{
